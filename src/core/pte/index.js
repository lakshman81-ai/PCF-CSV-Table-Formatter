...
function mapToDataTable(elements, config) {
  // elements is an array of objects resembling the Data Table schema,
  // generated by caseA/B/D modules
  const lineKeyTarget = config?.pte?.lineKeyTarget || "PIPELINE-REFERENCE";

  return elements.map((el, i) => {
    // Determine where to map the Line_Key if present
    const outPipelineRef = lineKeyTarget === "PIPELINE-REFERENCE" ? (el.pipelineRef || el.Line_Key || "") : (el.pipelineRef || "");
    const ca = el.ca || {1:null, 2:null, 3:null, 4:null, 5:null, 6:null, 7:null, 8:null, 9:null, 10:null, 97:null, 98:null};

    if (el.Line_Key) {
        if (lineKeyTarget === "CA1") ca[1] = el.Line_Key;
        if (lineKeyTarget === "CA98") ca[98] = el.Line_Key;
    }
