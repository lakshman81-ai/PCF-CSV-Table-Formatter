...
export function runValidation(dataTable, config, log) {
  ...
  for (let i = 0; i < dataTable.length; i++) {
    ...
    const isEnabled = (ruleId) => config.validator?.[ruleId] !== false;
    ...
    if (type === "BEND") {
      if (isEnabled("V4") && row.cp && row.ep1 && vec.dist(row.cp, row.ep1) < 0.1) addResult("V4", "ERROR", row._rowIndex, `ERROR [V4]: BEND CP = EP1`);
      if (isEnabled("V5") && row.cp && row.ep2 && vec.dist(row.cp, row.ep2) < 0.1) addResult("V5", "ERROR", row._rowIndex, `ERROR [V5]: BEND CP = EP2`);

      if (row.cp && row.ep1 && row.ep2) {
        const distToLine = pointToLineDistance(row.cp, row.ep1, row.ep2);
        if (isEnabled("V6") && distToLine < 0.1) {
        ...
        if (isEnabled("V7") && Math.abs(d1 - d2) > 1.0) {
    ...
    if (type === "TEE") {
      if (row.cp && row.ep1 && row.ep2) {
        ...
        if (isEnabled("V8") && vec.dist(row.cp, mid) > 0.1) {
      ...
      if (isEnabled("V10") && row.cp && row.bp && row.ep1 && row.ep2) {
    ...
    if (type === "SUPPORT") {
      ...
      if (row.ca?.[97] || row.ca?.[98]) hasCAs = true;
      if (row.ca?.[1]) hasCAs = true;
      if (isEnabled("V12") && hasCAs) addResult("V12", "ERROR", row._rowIndex, `ERROR [V12]: SUPPORT should not have CAs.`);

      if (isEnabled("V13") && row.bore && row.bore !== 0) addResult("V13", "ERROR", row._rowIndex, `ERROR [V13]: SUPPORT bore must be 0.`);

      if (isEnabled("V19") && row.text && (row.text.includes("LENGTH=") || row.text.includes("WEIGHT="))) {
         addResult("V19", "WARNING", row._rowIndex, `WARNING [V19]: SUPPORT MSG-SQUARE has LENGTH/WEIGHT token.`);
      }
    }

    // V14: SKEY
    if (isEnabled("V14") && type !== "PIPE" && type !== "SUPPORT" && type !== "MESSAGE-SQUARE" && type !== "PIPELINE-REFERENCE") {
      if (!row.skey) addResult("V14", "WARNING", row._rowIndex, `WARNING [V14]: Component missing <SKEY>.`);
    }

    // V15: Coordinate Continuity
    if (isEnabled("V15") && i > 0 && type === "PIPE") {
    ...
    // V20: GUID Prefix
    if (isEnabled("V20") && row.supportGuid && !row.supportGuid.startsWith("UCI:")) {
